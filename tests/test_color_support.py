...
def test_env_force_color__empty_string() -> None:
    assert _get_env_force_color() == ColorMode.ANSI_16
...
def test_env_force_color__false() -> None:
    assert _get_env_force_color() == ColorMode.NO_COLOR
...
def test_env_force_color__2() -> None:
    assert _get_env_force_color() == ColorMode.EXTENDED_256


@mock.patch.dict(os.environ, {"FORCE_COLOR": "3"}, clear=True)
def test_env_force_color__3() -> None:
    assert _get_env_force_color() == ColorMode.TRUE_COLOR
...
def test_detect_color_support__respects_force_color_2() -> None:
    assert detect_color_support() == ColorMode.EXTENDED_256


@mock.patch.dict(os.environ, {"FORCE_COLOR": "3"}, clear=True)
def test_detect_color_support__respects_force_color_3() -> None:
    assert detect_color_support() == ColorMode.TRUE_COLOR
...
def test_detect_color_support__windows_broken_version_1() -> None:
    assert detect_color_support(sys.stdout) == ColorMode.ANSI_16
...
def test_detect_color_support__windows_min_build_for_256_colors() -> None:
    assert detect_color_support(sys.stdout) == ColorMode.EXTENDED_256
...
def test_detect_color_support__windows_min_build_for_true_colors() -> None:
    assert detect_color_support(sys.stdout) == ColorMode.TRUE_COLOR
...
@mock.patch.dict(
    os.environ,
    {"TERM_PROGRAM": "iTerm.app", "TERM_PROGRAM_VERSION": "2.9.3"},
    clear=True,
)
@mock.patch("sys.stdout.isatty", lambda: True)
...
@mock.patch.dict(
    os.environ,
    {"TERM_PROGRAM": "iTerm.app", "TERM_PROGRAM_VERSION": "3.0.10"},
    clear=True,
)
@mock.patch("sys.stdout.isatty", lambda: True)
...
@mock.patch.dict(
    os.environ,
    {"TERM_PROGRAM": "iTerm.app", "TERM_PROGRAM_VERSION": "broken"},
    clear=True,
)
@mock.patch("sys.stdout.isatty", lambda: True)
